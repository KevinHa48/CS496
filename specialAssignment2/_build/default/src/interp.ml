...
let lookup_class : string -> class_env -> class_decl ea_result  = fun c_name c_env ->
  if (List.exists(fun (name, _) -> name = c_name) c_env)
  then return @@ (snd (List.find(fun (name, _) -> name = c_name) c_env))
  else error "lookup_class: class c_name not found"
...
and
  ...
  | Send(e,m_name,es) ->
    eval_expr e >>=
    obj_of_objectVal >>= fun (c_name, env) ->
    let self = ObjectVal(c_name, env) in
    sequence (List.map eval_expr es) >>= fun args ->
