...
declaration")
  ...
  | NewRef(e) ->
    type_of_expr e >>= fun exp ->
    return @@ RefType(exp)  
  | DeRef(e) ->
    type_of_expr e >>=
    arg_of_refType "deref: " >>= fun s ->
    return s
  | SetRef(e1,e2) ->
    type_of_expr e1 >>=
    arg_of_refType "setref: " >>= fun _ ->
  ...
  | Hd(e) ->
    type_of_expr e >>=
    arg_of_listType "hd: " >>= fun tp ->
    return tp
  | Tl(e) ->
    type_of_expr e >>=
    arg_of_listType "tl: " >>= fun tp ->
    return @@ ListType(tp)
  (* trees *)
  | EmptyTree(t) ->
    return @@ TreeType(t) 
  | Node(de, le, re) ->
    type_of_expr de >>= fun det ->
    type_of_expr le >>= 
    arg_of_treeType  "node: " >>= fun t1 ->
    type_of_expr re >>= 
    arg_of_treeType  "node: " >>= fun t2 ->
    if (det=t1 && t1=t2)
    then return @@ TreeType(det)
    else error "Node: element or tree types do not match"
  | NullT(t) ->
    type_of_expr t >>=
  ...
  | GetData(t) ->
    type_of_expr t >>=
    arg_of_treeType "getdata: " >>= fun tp ->
    return tp  
  | GetLST(t) ->
    type_of_expr t >>=
    arg_of_treeType "getlst: " >>= fun tp ->
    return @@ TreeType(tp)    
  | GetRST(t) ->
    type_of_expr t >>=
    arg_of_treeType "getrst: " >>= fun tp ->
    return @@ TreeType(tp)  
  | Debug(_e) ->
